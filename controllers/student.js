...
router.post('/register/add', function(req, res) {
  let checksql =
    'SELECT * FROM sections NATURAL JOIN register \
          WHERE course_id = ? AND\
                section_id = ? AND\
                year = ? AND\
                semester = ? ; ';
  let sql =
    "INSERT INTO register (`course_id`,`section_id`,`year`,`semester`,`student_id`,`grade`)\
     VALUES (?, ?, ?, ?,?, '-') ";

  console.log(req.body);

  mysql.query(
    'SELECT * FROM register WHERE course_id = ? AND year = ? AND semester = ? AND student_id = ?',
    ...
    (err, result) => {
      if (result.length > 0) res.send('EXIST');
        ...
        mysql.query(
          checksql,
          [req.body.course_id, req.body.section_id, req.body.year, req.body.semester],
          function(err, result) {
...
router.post('/request', function(req, res, next) {
  if (!req.session.isLogin || req.session.userType != 'student') res.send('FAIL');
    ...
    if (req.body.type) {
      let query = `INSERT INTO requests (student_id, type) VALUES ('${req.session.userID.toString()}', '${
        req.body.type
      }')`;
...
router.get('/course/all', function(req, res) {
  ...
  const query =
    'SELECT * FROM (register NATURAL JOIN (sections NATURAL JOIN time_slots)) NATURAL JOIN courses WHERE student_id = "' +
    student_id +
    '"';
...
router.post('/register/pre_add', function(req, res) {
  let sql =
    'INSERT INTO pre_register (`course_id`,`section_id`,`year`,`semester`,`student_id`,`plan_id`)\
     VALUES (?, ?, ?, ?, ?, ?) ';
...
router.post('/register/pre_all', function(req, res) {
  ...
  Promise.all(promises)
    .then(() => {
      mysql.query(
        `DELETE FROM pre_register WHERE student_id = ${student_id} AND plan_id = ${plan_id} AND year = 2017 AND semester = 2`,
        (err, result) => {
          if (err) res.send('FAIL');
          else res.send('OK');
        }
  ...
  const promise = new Promise((resolve, reject) => {
    mysql.query(query, function(err, timeSlots) {
      if (err) reject(err);
        ...
        mysql.query(
          `SELECT * FROM (register NATURAL JOIN (sections NATURAL JOIN time_slots)) NATURAL JOIN courses WHERE student_id = ${student_id} AND year = 2017 AND semester = 2`,
          (err, result) => {
            if (err) reject(err);
